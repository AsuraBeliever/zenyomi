...
import mihon.data.animeextension.model.NetworkAnimeExtensionStore.ContentWarning
import mihon.data.animeextension.model.NetworkAnimeExtensionStore.ExtensionList
import mihon.domain.extension.model.ExtensionStore
import eu.kanade.tachiyomi.animeextension.model.AnimeExtension as ZenyomiAnimeExtension
...
fun ExtensionList.toAvailableExtensions(store: ExtensionStore): List<ZenyomiAnimeExtension.Available> {
    return extensions.map { extension ->
        val lang = extension.sources.map { it.language }.toSet()
        ZenyomiAnimeExtension.Available(
            name = extension.name,
            ...
            sources = extension.sources.map { source ->
                ZenyomiAnimeExtension.Available.Source(
                    id = source.id,
                    name = source.name,

...
## [0.19.3] - 2026-09-19
### Fixed
- **El salto ya no cae pasado el final del opening cuando los tiempos vienen de otra copia del episodio.** Los segundos de AniSkip se miden sobre el fichero que alguien cronometró; si el stream que estás viendo no es exactamente esa copia —unos segundos de logo de más, un montaje cortado distinto—, el intervalo dura lo que debe pero está corrido, y el salto caía unos segundos después del final del opening llevándose episodio por delante. Ahora aterriza siempre un poco corto, y cuanto menos se parezcan las dos copias —la respuesta dice sobre qué duración se midió—, más corto. Con tope, y sin margen ninguno cuando el dato sale de los capítulos del propio fichero, que hablan de la copia que se está viendo.
- **Omitir el opening ya no cuesta episodio por tardar en pulsar.** Cuando nadie sabe dónde acaba el opening, el botón saltaba la cantidad configurada *desde donde se pulsaba*: pulsarlo diez segundos después de empezar el opening aterrizaba diez segundos pasado su final, y esos diez segundos de episodio se perdían. Un opening dura 85 segundos **contados desde su principio**, así que ahora se miden desde ahí: sin datos del episodio se asume lo que ya asume el propio botón —que el opening empieza con el episodio—, y pulsar en el segundo 5 o en el 50 aterriza en el mismo sitio. Puede dejar unos segundos de opening sonando cuando el episodio abre con una escena; ese es el lado correcto por el que equivocarse. Y nunca aterriza pasado el último segundo del episodio, que antes podía encadenar con el siguiente.
...
- **Los tiempos reales del opening, cuando se saben.** Si el anime está vinculado a MyAnimeList o AniList, Zenyomi le pregunta a AniSkip dónde empieza y acaba el opening de ese episodio: el botón deja de saltar una cantidad fija y cae justo donde termina, y solo se ofrece mientras el opening está en pantalla. Los créditos también se saben, y son los que anuncian el siguiente episodio. Se puede apagar en Ajustes → Reproductor, junto a un salto automático que viene desactivado. Lo que sale del móvil al preguntar está escrito en `docs/adr/0007-de-donde-salen-los-tiempos-del-opening.md`.
...
- **The track picker ticks the track you are hearing.** It also re-reads the list while the player is open, so an audio track that arrives late shows up selected rather than leaving a stale tick somewhere else. Picking a different track no longer shifts every label sideways.
- **A way out of a slow episode.** Opening one reaches the source over the network and can take the better part of ten seconds, which used to be a spinner on a single row while the rest of the screen still took taps — it read as "nothing happened" and invited a second tap somewhere else. Both the entry screen and the player now cover themselves while they load, swallow taps instead of queueing them, and let you leave with back or the close button.

### Fixed
- **The buttons work from the first second.** Pause, seek and track selection did nothing for the first half-minute of an episode and then did everything that had been pressed at once — pause pressed at 13:04:12, run at 13:04:30. Loading a subtitle blocks mpv for as long as its download takes, and a source offering sixteen languages held it for about a minute with every button press queued behind it. Only your preferred language is loaded up front now; the rest are still listed in the picker and cost about two seconds when picked. Choosing a language also sticks — applying the preference ran on every change to the track list, so picking English on a source that also had the preferred Spanish flipped back to Spanish a second later.
- **The sound starts with the picture.** On a KickAssAnime episode mpv's own log showed playback starting at 12:47:54 and the audio track selected at 12:48:32 — thirty-seven seconds of silent video. The audio was queued behind sixteen subtitle downloads, and the selection behind that. Audio is loaded first now, the preferred language ahead of the rest, and mpv is told to select the track as it registers it rather than being asked afterwards. Same episode: two seconds. The remaining pair is the audio stream being fetched, which is not ours to remove.
- **The clock keeps time.** It advanced in uneven jumps — 30, then 34 — because the position was polled every two seconds and each pass made several blocking reads of mpv first, so the period was two seconds plus however long mpv took to answer. It now ticks once a second, on the second.
- **The seek bar stays where it is put.** Dragging it showed the thumb at the destination, snapped back to where the episode was, then jumped forward again: mpv keeps reporting the old position until it has decoded the new one, and those reports were being applied. The bar holds its destination until mpv arrives there. A drag also used to raise the full loading cover and lock the screen; opening an episode and stalling mid-episode are now separate — the first blocks and offers a way out, the second is a spinner over the picture you were already watching.
